...
fn fill_live_input(r: &mut AppRenderer, parent_id: &sicompass_sdk::ffon::IdArray, value: &str) {
    ...
    let new_key = {
        ...
        let open = "<input>";
        let close = "</input>";
        match (key.find(open), key.find(close)) {
            (Some(op), Some(cp)) if cp >= op => format!(
                "{}{}{}",
                &key[..op],
                tags::format_input(value),
                &key[cp + close.len()..],
            ),
            _ => return,
        }
    };
    if let Some(arr) = crate::state::navigate_to_slice_pub(&mut r.ffon, parent_id) {
        if let Some(FfonElement::Obj(o)) = arr.get_mut(pidx) {
            o.key = new_key;
        }
    }
    // Persist on the provider so a tick()-driven re-fetch keeps the value.
    if let Some(p) = crate::provider::get_active_provider(r) {
        p.set_input_value(value);
    }
...
mod tests {
    ...
    fn tab_from_general_enters_simple_search() {
        ...
        assert_eq!(r.previous_coordinate, Coordinate::General);
        assert!(r.search_string.is_empty());
        // handle_tab passes the current item label as context ("dash item 0"
        // because make_renderer starts at list index 0 with label "- item 0").
        assert_eq!(
            announced_text(&r).as_deref(),
            Some("search mode - dash item 0")
        );
    }
